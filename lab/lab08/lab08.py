def make_to_string(front, mid, back, empty_repr):
    ...
    '()'
    """
    def print_the_link(lnk):
        if lnk is Link.empty:
            return empty_repr
        else:
            return front+ str(lnk.first) + mid + print_the_link(lnk.rest) + back
    return print_the_link
...
def tree_map(fn, t):
    ...
    changed_subtrees = [tree_map(fn, b) for b in t.branches]
    return Tree(fn(t.label), changed_subtrees)
