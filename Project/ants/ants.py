...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while i<self.min_range:
            ...
            i+=1
        while i<=self.max_range and current_place!=hive:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.place==None:
            new_bees_place=list(place.bees)
            for bees in new_bees_place:
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6A
        new_bees_place=list(self.place.bees)
        for bees in new_bees_place:
            bees.reduce_armor(self.damage)
        # END Problem 6A
        ...
        if self.impostor==True:
            ...
            while current_place.exit != None:
                ...
                if current_place.ant:
                    if not current_place.ant  in self.double :
                        current_place.ant.damage*=2
                        self.double.append(current_place.ant)

                    if current_place.ant.container and current_place.ant.ant:
                        if not current_place.ant.ant  in self.double:
                            current_place.ant.ant.damage*=2
                            self.double.append(current_place.ant.ant)  
