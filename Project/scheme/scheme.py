...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        lst_for = formals 
        lst_vals = vals  
        while lst_for is not nil and lst_vals is not nil:
            child.define(lst_for.first,lst_vals.first)
            lst_for = lst_for.second
            lst_vals = lst_vals.second
        # END PROBLEM 11
        return child
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        "*** REPLACE THIS LINE ***"
        new_operands=operands.map(lambda x :scheme_eval(x,env))
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        name_pair=Pair(bindings.first.first,name_pair)
        values_pair=Pair(scheme_eval(bindings.first.second.first,env),values_pair)
        check_form(bindings.first,2,2)
        check_formals(name_pair)
        bindings=bindings.second
    return env.make_child_frame(name_pair,values_pair)
    # END PROBLEM 15
