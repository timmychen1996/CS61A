...
def lt_rat(x, y):
    ...
    False
    """
    if make_rat(numer(x),denom(x))< make_rat(numer(y),denom(y)):
        return True;
    else:
        return False
...
def has_seven(k):
    ...
    """
    def Spilt(n):
        one_digit, another_digit=n//10 , n%10
        if another_digit==7:
            return True
        if one_digit==7:
            return True
        if one_digit<10 and one_digit!=7 :
            return False
        else:
            return Spilt(one_digit)
    return Spilt(k)
...
def pingpong(n):
    ...
    True
    """
    def continue_sequence(cur_number,cur_index,operator):
        if n==cur_index:
            return cur_number
        elif has_seven(cur_index)==False and cur_index%7!=0:
            return continue_sequence(cur_number+operator,cur_index+1,operator)
        else: 
            return  continue_sequence(cur_number-operator,cur_index+1,-operator)
    return continue_sequence(1,1,1)
...
def deep_len(lnk):
    ...
    if isempty(lnk):
        return 0
    if type(first(lnk))==list:
        return 2+deep_len(rest(lnk))
    else:
...
def riffle(deck):
    ...
    """
    middle=len(deck)//2
    return flatten([[deck[i],deck[middle+i]]for i in range (middle)])
