...
def simplify(expr):
    ...
    if expr.is_leaf():
        return expr
    left=simplify(expr.__getitem__(0))
    right=simplify(expr.__getitem__(1))
    if expr.label=='+':
        if left== ZERO:
            return (right)
        elif right==ZERO:
    ...
    if expr.label=='*':
        if right== ONE:
            ...
            return ZERO
    return (Expr(expr.label,[left, right]))
...
def dsimplify(expr):
    ...
    if expr.is_leaf():
        return expr
    left=dsimplify(expr.__getitem__(0))
    right=dsimplify(expr.__getitem__(1))
    if expr.label=='+':
        if left== ZERO:
            expr=right 
            return expr
    ...
    if expr.label=='*': 
        if right== ONE:
            ...
            return ZERO
    expr.__setitem__(0,left) 
    expr.__setitem__(1,right)
    return expr 
class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            self.balance = 0
            if self.change > 0:
                return 'Here is your ' + str(self.name) + ' and $' + str(self.change) + ' change.'
...
def merge(s0, s1):
    ...
    while e0 is not None or e1 is not  None:
        if e0 is None:
            if e1 not in old_list:
                old_list.append(e1)
                yield e1 
            e1=next(i0,None)
        elif e1 is None:
            if e0 not in old_list:
                old_list.append(e0)
                yield e0
            e0=next(i0,None)
        elif e0<e1:
